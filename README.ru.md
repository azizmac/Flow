...
- **Поиск.** Гибрид векторного и полнотекстового поиска по задачам, комментариям, проектам и людям на pgvector: находит по смыслу, а не по подстроке. Индекс обновляется в той же транзакции, что и правка. Пока доступен по API — строка поиска в интерфейсе следующим шагом.
...
- [x] Векторный поиск: индекс на pgvector, эмбеддер Qwen3, гибридная выдача с RRF (этапы 1–4 из `docs/TZ_search_vector.md`)
...
Клиентской строки поиска пока нет — она следующим этапом; сейчас поиск доступен по API.
