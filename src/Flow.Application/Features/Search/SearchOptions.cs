...
public sealed class SearchOptions
{
    ...
    /// «без моделей» должен гасить обе, иначе установка без карты продолжала бы гнать кадры в модель.
    /// </summary>
...
public sealed class SearchVisionOptions
{
    ...
    /// <summary>
    /// Бюджет ЗАПРОСА пользователя. Индексация ждёт дольше и своего значения здесь не имеет
    /// намеренно: её бюджет (две минуты) прибит в коде рядом с объяснением — первая картинка после
    /// загрузки модели идёт 31 с из-за сборки вычислительного графа, и это не то число, которое
    /// стоит отдавать в настройки как «подкрутите, если что».
    /// </summary>
