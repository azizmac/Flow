...
> Статус: сервис реализован в таком виде. В рамках перехода на модульный монолит `Flow.Auth` становится
> модулем `Flow.Api` (учётные записи — in-process, admin-API удаляется); актуальный план —
> `docs/TZ_modular_monolith.md`. Это ТЗ описывает исходную сервисную реализацию и остаётся историческим.
