...
/// <summary>Бросает ArgumentException, если поля не проходят валидацию (см. User.Create) или Auth-модуль отверг пароль.</summary>
    ...
    : IRequestHandler<UserCreateCommand, UserCreateResult>
{
    public async Task<UserCreateResult> Handle(UserCreateCommand request, CancellationToken cancellationToken)
    {
        ...
        // Локальная копия — быстрый отказ без обращения к Auth-модулю; источник истины всё равно там.
        ...
        switch (account.Status)
        {
            ...
            case AccountResultStatus.Invalid:
                throw new ArgumentException(account.Error ?? "Auth rejected the account.", nameof(request.Password));
        }
