...
- Ввод с клавиатуры фильтруется в `Components/DueDateField.razor`: нативное поле шлёт `change` на каждое
  полное значение, и набранный руками год успевает побывать «0002» и «0020». Поэтому поле ограничено
  `min`/`max` (±20 лет от сегодня плюс уже сохранённое значение), значения вне диапазона не отправляются,
  а пустое поле не считается «снять срок» — срок снимает только кнопка. Проигнорированный ввод не остаётся
  на экране: при потере фокуса поле пересоздаётся по `@key` и показывает сохранённую дату.
