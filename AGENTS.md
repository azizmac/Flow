...
- Docker, весь стек одной командой: `cp .env.example .env` (на машине с локальным Postgres — `POSTGRES_PORT=5433`), затем `docker compose up -d --build` → клиент http://localhost:5016, Api :8080, Auth :5100, вход `admin@flow.com` / `admin`. Сертификаты Flow.Auth создаются сами в volume `auth-certs`, базы `flow`/`flow_auth` — миграциями при старте. Клиент — `Dockerfile.client` (nginx, `docker/client/entrypoint.sh` пишет `appsettings.json` из `API_BASE_URL`/`AUTH_BASE_URL`). Api внутри compose берёт discovery/JWKS по `http://auth:8080`, а issuer проверяет внешний (`Auth/IssuerRewritingConfigurationRetriever`), `Auth__RequireHttpsMetadata=false`. Только Postgres: `docker compose up -d postgres`. Базовый пользователь — якорь `x-bootstrap` в compose (`BOOTSTRAP_*`).
...
- CI: GitHub Actions — `ci.yml` (триггеры и порядок) вызывает `build.yml` (restore → build Release → юнит-тесты → интеграционные на Testcontainers) и `docker.yml` трижды (образы `Flow.Api` по `Dockerfile`, `Flow.Auth` по `Dockerfile.auth`, `Flow.Client` по `Dockerfile.client`; с `main` публикуются в registry по секретам `REGISTRY_USERNAME`/`REGISTRY_PASSWORD` и переменным `REGISTRY`/`IMAGE_NAME`/`AUTH_IMAGE_NAME`/`CLIENT_IMAGE_NAME`).
